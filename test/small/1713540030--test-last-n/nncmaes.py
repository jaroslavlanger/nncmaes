                ...
                if ep_ % (n_epochs / 5) == 0:
                    ...
                    if __debug__ and self.__debug:
                        print(
                            ...
                            "tau-train={:>5.2f}".format(kendalltau(y_train[-_n_kendall_archive:], np.array(NNs[ens].predict(X_train[-_n_kendall_archive:], sess))).statistic),
                        )
...
def test(
    *,
    ...
    models = [predict_zeros_and_ones, predict_random, Raf(data_noise=None, debug=True)][2:]
    surrogates = [
        Surrogate(
            model=model,
            subset=LastN(),
